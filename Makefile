...
update-token-env:
	@sed -i .`date +%Y%m%d`.bak "s/REPLACE_ONCE_GENERATED/`docker exec openslava-gitlab-ce gitlab-rails runner -e production "puts Gitlab::CurrentSettings.current_application_settings.runners_registration_token"`/g" .env

runner-register: update-token-env
	@bash scripts/register-runner.sh

personal-token:
	@docker exec openslava-gitlab-ce gitlab-rails runner "token = User.find_by_username('root').personal_access_tokens.create(scopes: [:api], name: 'terraform'); token.set_token('my-tf-token-2022'); token.save"
...
vault-configure: build-ansible-image
	@docker run --rm -it -v ${PWD}/vault:/vault --network=toolchain-network openslava-ansible:1.0 ansible-playbook /vault/playbooks/main.yml
