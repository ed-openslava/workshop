...
To initialise Vault, yet to configure it in a basic way there are a few steps ahead. But no worries, if you are persistent enough you may very well automate the effort away. 
...
There are a few lines of ansible code prepared for you to initialise and configure Vault. 
...
Once your build is done try to run the playbook inside your newly build container to configure initialise and configure Vault for you.

```
docker run --rm -it -v ${PWD}:/vault --network=toolchain-network openslava-ansible:1.0 ansible-playbook /vault/playbooks/main.yml
```
